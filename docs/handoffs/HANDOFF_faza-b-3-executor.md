...
> **Status:** In progress (B3.0-B3.7 ALL DONE 2026-05-30; ADR-0011 Accepted; GATE OPEN; B3.8 weryfikacja end-to-end next)
...
### B3.3 — Prediction (analytic, no RNG) — **DONE 2026-05-30**

- [x] `app/services/engine/prediction.py` (~210 LOC): `_success_probability(threshold, modifier, *, natural_6_auto_success, natural_1_auto_failure)` primitive per pkt 1; `_binomial_pmf(n, p)` z math.comb (suma=1 invariant); `DamageDistribution` frozen (pmf/mean/models_at_risk/toughness_per_model) z helpers `p_at_least(n)`/`p_kill()`/`p_full_kill()`/`expected_models_killed()` (capped at models_at_risk).
- [x] `expected_damage(attacker, defender, weapon, terrain=())` — analityczny equivalent `combat.resolve_ranged_attack` (REUSES `compute_cover`/`compute_attack_modifiers`/`compute_defense_modifier` z combat.py dla spójności). `p_wound = p_hit * (1 - p_save)` → Binomial(n_attacks, p_wound) → PMF + mean. MVP scope: AP/Brutalny/Precyzyjny.
- [x] `would_see(attacker_position, attacker_radius, target, terrain)` — hipotetyczny LoS z ephemeral UnitBlob → delegate `los.check_los`. Dla heuristic players (Strumień D) i prediction-time queries.
- [x] Testy `tests/test_prediction_vs_simulation.py` (38 testów): `_success_probability` 7 (thresh 2-7, Brutalny, modifier clamp), `_binomial_pmf` 6 (sum=1, n=0, p=0/1, validation), `DamageDistribution` 5 (p_at_least/p_kill/p_full_kill/expected_killed/capped), `expected_damage` 6 (basic/no attacks/high Q low D/AP↑/Brutalny↑/Cover↓), `would_see` 2, **Monte Carlo parity 8 scenariuszy × 500 sym + cover scenario** — wszystkie pass ±3σ tolerance (ADR-0044 inwariant verified).
- [x] ADR-0044 (`docs/adr/0044-prediction-module.md`) Status: Accepted — analytic binomial bez RNG dla agents/MCP, REUSES combat helpers (consistency invariant). 6 alternatyw odrzuconych (MC na żądanie/cached MC/mean only/direct math/PGM/scipy). Planowani konsumenci: greedy_player/minimax_player/policy_eval/simulate_engagement/find_exploit_candidates. 4 inwarianty replay.
...
### B3.5 — Effects + Interrupts — **DONE 2026-05-30** (framework + 3 passives + 1 example interrupt; rozszerzenie w B3.6+)

- [x] `app/services/engine/effects.py` (~210 LOC): per-hook registry framework (`_DEFENSE_MODIFIERS`, `_ATTACK_MODIFIERS`, `_MORALE_MODIFIERS`, `_WEAPON_MODIFIERS`). `EffectContext` frozen (blob/state/weapon/is_charging/is_being_charged). Decorator-y `register_*_modifier(slug)`. Aggregators: `aggregate_defense_modifier(ctx)`, `aggregate_attack_modifier(ctx)`, `aggregate_morale_modifier(ctx)`, `apply_weapon_modifiers(weapon, blob)`.
- [x] Konkretne passive abilities MVP (3): **Cierpliwy** (id 3, +1 defense gdy nie Aktywowany), **Tarcza** (id 34, +1 defense gdy nie Przyszpilony), **Nieustraszony** (id 16, -1 morale test). Pozostałe 40+ passive abilities (Furia/Impet/Maskowanie/Niewrazliwy/Delikatny/Okopany/Parowanie/Przygotowanie/Regeneracja/Odrodzenie/Roj/Szpica/Waagh/Zdobywca/Instynkt/Dywersant/Niestrudzony/Ochroniarz/Niezgrabny/Harcownik/Latający/Zwinny/Bohater/Bastion/Kontra/Niezawodny/Dobrze/Źle strzela/Ostrożny/Niezgrabny/Nieruchomy/Mistrzostwo/Stracency/Szybki/Wolny/Transport/Zasadzka/Zwiadowca/Odwody/Rezerwa/etc.) → dorzucane stopniowo w B3.6+ przy integracji z phases/combat.
- [x] `app/services/engine/interrupts.py` (~150 LOC): `InterruptPoint` Enum (4 wartości per ADR-0015), `InterruptContext` frozen (state/point/source_unit_id/active_unit_id), `register_interrupt_handler(point, slug)` decorator z duplicate guard, `get_eligible_interrupts(state, point)` filtr defeated + per-slug dispatch, `trigger_interrupt(context, slug, payload)` dispatch. 1 example handler: **Strażnik** (id 31) at `ACTIVATION_START` jako **stub** — emit `InterruptTriggered` event tylko (faktyczny Ostrzał + Wyczerpany status w B3.6 phases integration).
- [x] Testy `tests/test_engine_effects.py` (21): EffectContext frozen, Cierpliwy 3 cases, Tarcza 2, multiple sum, partial conditions, Nieustraszony 2, empty/no-op aggregators 3, weapon_modifier no-op, duplicate registration raises, registry sanity 5.
- [x] Testy `tests/test_engine_interrupts.py` (17): enum 2, InterruptContext frozen, get_eligible 6 (empty/strażnik at AS/not at other/no passive/defeated/multiple), trigger 5 (calls handler/unknown raises/wrong point raises/payload propagated/stub no mutation), duplicate raises, registry sanity 2.
- [x] ADR-0015 (`docs/adr/0015-interrupt-points.md`) Status: Accepted — 4 zamknięte punkty per pkt 12 (ACTIVATION_START/AFTER_ACTION/BEFORE_REGROUP/AFTER_REGROUP), 5 constraints (zamknięta lista, atomowy, no nested, per-runda limity, inicjatywa), mapping 6 aktywnych zdolności (Rozkaz/Klątwa/Oznaczenie/Usprawnienie/Koordynacja/Przekaźnik) + Strażnik. 5 alternatyw odrzuconych. 4 inwarianty replay.

### B3.6 — Phases (round flow) — **DONE 2026-05-30**

- [x] `app/services/engine/actions.py` (~80 LOC): action types frozen — `DeploymentAction`, `ManeuverAction`, `DefendAction`, `ShootAction`, `ChargeAction`, `SpecialAction`. `Action` Union alias.
- [x] `app/services/engine/state.py` rozszerzenie: `Objective(id, position, controller)` frozen + `BattleState` field `objectives` + `is_game_over` + `score`.
- [x] `app/services/engine/phases.py` (~420 LOC):
  - `setup_phase(rosters, terrain, objectives, initiative_player, ruleset_version)` — pkt 7 + 9; delegacja do `build_initial_state` (ADR-0008 exclusion validation), set `active_player=initiative`.
  - `deployment_round(state, deployment_actions)` — pkt 13: emit MoveExecuted move_type="deploy" per akcja, po końcu reset Aktywowany + round=1.
  - `activation_phase(state, action, dice)` — pkt 11.b: dispatch per Action type → akcja → Przegrupowanie pkt 20 (gdy wounds_received>0 lub melee_balance<0) → Aktywowany status. Sub-helpers: `_apply_maneuver`/`_apply_defend`/`_apply_shoot`/`_apply_charge`/`_apply_special` (delegacja do combat.resolve_*). `_regroup_test` uwzględnia: bazę 1 + pkt 20.b (≤½ wytrzymałości) + pkt 20.c (melee_balance) + pkt 20.d (status: Przyszpilony +1, Ufortyfikowany -1) + passive morale modifiers (Nieustraszony -1 z effects.py). Wyniki: 1 fail → Wyczerpany, 2 → Wyczerpany+Przyszpilony, ≥3 → pokonany.
  - `round_end_phase(state)` — pkt 8.c: reset Aktywowany, `_check_objective_control` (pkt 5.d — 3″ od celu, tylko owner's units, Przyszpilony nie kontroluje pkt 22.b.ii, disputed → previous controller pozostaje), emit RoundEnded, round+1, `is_game_over=True` po round 4 (pkt 5.f).
- [x] Pkt 14.e SpecialAction MVP: `discard_exhausted` (uniwersalny pkt 22.a.ii) impl; inne aktywne zdolności (Łatanie/Mag/Mobilizacja/etc.) jako noop event placeholder (full impl w przyszłych iteracjach przez effects.py ACTIVE_ABILITY_REGISTRY).
- [x] Testy `tests/test_engine_phases.py` (25 testów): setup 3 (state/initiative/objectives), deployment 3 (move/round inc/status reset), activation 7 (maneuver/defend/defend-removes-pinned/shoot/charge/special-discard/unknown raises), regroup 3 (skipped no wounds/triggered with wounds/Nieustraszony reduces count), round_end 5 (reset Aktywowany/round inc/RoundEnded event/game_over round 4/no game_over round 3), objective control 4 (unclaimed/single player controls/disputed keeps previous/Przyszpilony nie kontroluje).
- [x] Pytest **1222/1222** (1197 baseline + 25 nowych z B3.6).

### B3.7 — Resolver (top-level entry) — **DONE 2026-05-30**

- [x] `app/services/engine/resolver.py` (~140 LOC): `apply(state, action, dice, sequence) → ResolverResult` — **pure function** dispatcher. `ResolverResult(state, events, next_sequence)` frozen. `IllegalActionError` z 5 powodami (game over, not found, defeated, already Aktywowany, wrong active_player).
- [x] Switch active_player po aktywacji (pkt 8.a) — `_switch_active_player`: przechodzi na przeciwnika **chyba że** brak nieaktywowanych żywych oddziałów (fallback).
- [x] Helpers: `should_end_round(state)` (pkt 8.b — True gdy wszyscy żywi Aktywowani), `is_battle_over(state)` (alias state.is_game_over).
- [x] `Action` schema z B3.6 actions.py (ManeuverAction/DefendAction/ShootAction/ChargeAction/SpecialAction). `isinstance` dispatch w `phases.activation_phase` (decyzja: bez Pydantic discriminated union — vintage clean per ADR-0011).
- [x] Testy `tests/test_engine_resolver.py` (22 testów): basic per Action type (4), walidacja IllegalActionError (5), switch active_player (3), should_end_round + is_battle_over (5), deterministic replay (2), pure function (1), smoke 2v2 (2: completes + game over after round 4).
- [x] ADR-0011 promote Proposed → **Accepted** z empirycznymi wnioskami (8 punktów rozstrzygniętych w B3.1-B3.7: effects size OK, combat complexity OK, isinstance polimorfizm, full test coverage 260+ testów, ortogonalność z lazy import dla combat↔effects, perf <1ms typowy, reactive window stable, replay determinism verified). Public API zdefiniowane.
- [x] Pytest **1244/1244** (1222 baseline + 22 nowych z B3.7).
...
- 2026-05-30 (post-B3.5): **B3.5 effects + interrupts framework zamknięte.** `app/services/engine/effects.py` (~210 LOC) z per-hook registries (defense/attack/morale/weapon), EffectContext frozen, decorator-y register_*_modifier + aggregate_* helpers. 3 representative passives (Cierpliwy/Tarcza/Nieustraszony). `app/services/engine/interrupts.py` (~150 LOC) z InterruptPoint enum (4 wartości), InterruptContext, register/get_eligible/trigger framework. Strażnik (id 31) MVP stub. **38 nowych testów** (21 effects + 17 interrupts). ADR-0015 Accepted (4 zamknięte punkty per pkt 12, no nested, no async, framework dla 6 aktywnych zdolności + Strażnik). Pytest **1140/1140** (1102 + 38). Następne kroki: **B3.6 phases** (setup_phase/deployment_round/activation_phase/round_end_phase, integracja effects+interrupts+combat) lub **B3.3 prediction** (analytic expected_damage).
- 2026-05-30 (post-B3.3): **B3.3 prediction zamknięte.** `app/services/engine/prediction.py` (~210 LOC) z analytic binomial — `_success_probability` (pkt 1 primitive), `_binomial_pmf` (math.comb), `DamageDistribution` frozen (pmf/mean/p_at_least/p_kill/p_full_kill/expected_models_killed), `expected_damage` REUSES combat.compute_cover/compute_attack_modifiers/compute_defense_modifier dla spójności semantyki (MVP scope: AP/Brutalny/Precyzyjny), `would_see` z ephemeral blob → delegate los.check_los. **38 nowych testów** włącznie z **Monte Carlo parity 8 scenariuszy × 500 sym + cover** — analytic.mean within ±3σ simulated.mean (ADR-0044 inwariant). ADR-0044 Accepted (analytic binomial dla agents/MCP, 6 alternatyw odrzuconych, 4 inwarianty). Bugfix przy implementacji: Brutalny test wymaga AP>0 żeby effective_threshold > 6 (przy threshold=6 i bez AP, normal 6+ sukces NIE jest auto-success ale jest normalnym sukcesem 6≥6 — Brutalny nie zmienia). Pytest **1178/1178** (1140 + 38). Następne kroki: **B3.6 phases** (compose effects + combat + interrupts) lub **B3.7 resolver** (top-level apply dispatcher). Po B3.6+B3.7 → promote ADR-0011 Proposed→Accepted + B3.8 weryfikacja end-to-end.
- 2026-05-30 (post-B3.4 extension): **B3.4 extension zamknięte.** 3 sub-zakresy: **(a) combat ↔ effects integration** — combat.py i prediction.py wywołują `aggregate_attack_modifier` + `aggregate_defense_modifier` z effects.py; passive Cierpliwy/Tarcza faktycznie modyfikują rolls (lazy import unika cyclic dependency). **(b) `resolve_charge_attack`** — pełna sekwencja pkt 14.d.i-vi: faza 1 Związanie (MoveExecuted move_type="binding") z pozycją końcową 1″ od defendera; faza 2 reactive window kontrataku pkt 14.d.iv (per ADR-0015a: jednorazowy/atomowy/no nested, defender wykonuje resolve_melee_attack jako attacker, po kontrataku Wyczerpany — chyba że Bastion id 1); faza 3 główny atak charger pkt 14.d.iii (pominięty jeśli charger pokonany). `ChargeResult` frozen + parametr `counter_attack_declared`. **(c) weapon abilities: Niezawodny + Podwójny** — `effective_attack_quality(weapon, attacker)` helper (Niezawodny id 63 → Q=2 priority); `_apply_podwojny_extra_hits` post-process inspekcji RollResult.rolls (Podwójny id 66 → +1 hit per natural 6). Prediction.py uwzględnia Podwójny przez `effective_p_hit = min(1, p_hit + 1/6)`. **19 nowych testów** (3 effects integration + 9 charge + 7 weapon abilities). Pytest **1197/1197** (1178 + 19). Następne kroki: **B3.6 phases** (compose activation_phase z effects/interrupts/combat), albo dalsze rozszerzenia B3.4 (Furia/Impet/Kontra/Strażnik full impl).
- 2026-05-30 (post-B3.6): **B3.6 phases zamknięte.** `app/services/engine/actions.py` (~80 LOC) — 6 Action types frozen + Union alias. `app/services/engine/state.py` rozszerzony o `Objective` + `BattleState.objectives/is_game_over/score`. `app/services/engine/phases.py` (~420 LOC) z 4 public functions: `setup_phase` (pkt 7+9), `deployment_round` (pkt 13, emit MoveExecuted move_type="deploy"), `activation_phase` (pkt 11.b dispatch per akcji → Przegrupowanie pkt 20 → Aktywowany), `round_end_phase` (pkt 8.c reset Aktywowany + `_check_objective_control` pkt 5.d + RoundEnded + `is_game_over` po round 4 pkt 5.f). Przegrupowanie uwzględnia wszystkie modyfikatory pkt 20 (b/c/d) + passive morale (Nieustraszony). Objective control wyłącza Przyszpilonych (pkt 22.b.ii). **25 nowych testów**. Pytest **1222/1222** (1197 + 25). Następny krok: **B3.7 resolver** (top-level `apply(state, action, dice)` dispatcher, Action polimorfizm z Pydantic discriminated union albo isinstance). Po B3.7 → promote ADR-0011 Proposed→Accepted + B3.8 weryfikacja end-to-end (smoke replay).
- 2026-05-30 (post-B3.7): **B3.7 resolver zamknięte + ADR-0011 Accepted.** `app/services/engine/resolver.py` (~140 LOC) — public engine entry: `apply(state, action, dice, sequence) → ResolverResult(state, events, next_sequence)`. Walidacja `IllegalActionError` z 5 powodami (game over, not found, defeated, already Aktywowany, wrong active_player). Switch active_player pkt 8.a + fallback gdy przeciwnik bez nieaktywowanych. Helpers `should_end_round(state)` (pkt 8.b) + `is_battle_over(state)`. Action dispatch przez `isinstance` w phases.activation_phase (decyzja per ADR-0011: bez Pydantic discriminated union — vintage clean). **22 nowych testów** włącznie z **smoke 2v2 battle end-to-end** + game over after round 4. ADR-0011 promote Proposed → Accepted z empirycznymi wnioskami (8 punktów rozstrzygniętych w B3.1-B3.7) + zdefiniowane Public API engine. Pytest **1244/1244** (1222 + 22). Następne kroki: **B3.8 weryfikacja end-to-end** (drift gate, baseline perf), **Strumień D** może startować (random/greedy player z prediction), **Strumień C** wymaga B2 + B4 najpierw.
