...
- [x] Krok A2.6: `docs/adr/0004-cost-dsl.md` (NOWY) — ADR podsumowujący decyzje DSL: hardcoded dispatcher (nie eval), callable injection (`passive_cost_fn`, `slug_for_name`), inwarianty czystości (no-oracle-import), tolerancja 1e-3, świadome odchylenie `transport_multiplier` priority-first vs oracle last-match-wins (parity-bug fix).
