...
- **2026-07-10 (wątek demoralizacja-mag, `/security-review`): fix bezpieczeństwa w `app/routers/collections.py`, poza zakresem tego wątku, ale w pliku zablokowanym przezeń — dokumentuję tu dla koordynacji.** `POST /collections/units/{unit_id}/models/add` nie miał sprawdzenia własności jednostki (w przeciwieństwie do sąsiedniego `GET /collections/units/{unit_id}`, który je miał) — dowolny zalogowany użytkownik mógł POST-ować z cudzym `unit_id` z prywatnej armii innego użytkownika, dostać w JSON nazwy broni/zdolności tej jednostki (`Accept: application/json`) i utrwalić `CollectionModel` odwołujący się do cudzej jednostki. Naprawa: nowy helper `_require_unit_access(unit, current_user)` (lustro `_require_owner`), użyty w obu miejscach ładujących `Unit` po `unit_id` z path param (`unit_collection` zrefaktoryzowany żeby go używać zamiast inline checka, `add_collection_model` dostał brakujący check). Trzeci load `Unit` (przy `update_collection_model`, linia ~433) NIE wymaga tego checka — tam `unit_id` pochodzi z już zweryfikowanego przez `_require_owner(cm, ...)` `CollectionModel`, nie z surowego path param. Regresja: `tests/test_collections_authz.py` (3 testy, zweryfikowane że failują bez fixa przez `git stash` na `collections.py`). Pytest całości: 280/280.
