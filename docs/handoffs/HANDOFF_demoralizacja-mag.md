...
## Faza H — Stan bitewny trudność + podgląd list zaklęć dla armii view-only (zgłoszone 2026-06-28)
- [x] H.1: `roster_battle_state.html` — dodano `<span class="spell-difficulty">{{ spell.difficulty }}+</span>` zaraz po `.spell-cost` (dane już płynęły przez `_army_spell_entries`, brakowało tylko renderu) + styl CSS `.battle-state .spell-difficulty`.
- [x] H.2: Armie view-only (owner_id=None, niebędący adminem użytkownik) — `GET /armies/{id}/spells` zmieniony z `_ensure_army_edit_access` na `_ensure_army_view_access` (mutujące trasy POST bez zmian, dalej `_ensure_army_edit_access`). Nowy helper SSOT `_army_can_edit(army, user)` (użyty też w `_render_army_edit`, zastępując zduplikowaną inline logikę). `_spell_page_context` zwraca `can_edit`.
- [x] H.3: `army_spells.html` — tryb read-only gdy `not can_edit`: brak uchwytu przeciągania/przycisków ↑↓, brak kolumny akcji (edytuj/usuń), nazwa własna jako tekst (nie formularz), brak karty „Dodaj/Edytuj moc", brak inicjalizacji Sortable.js, link „Zasady armii" ukryty (ta strona nadal wymaga edit-access). `army_edit.html` — link „Lista zaklęć" wyjęty spod `{% if can_edit %}` (widoczny dla wszystkich z dostępem do podglądu armii).
- [x] H.4: Testy `tests/test_army_spell_view_access.py` (4): dostęp do trasy GET dla view-only vs edit-only, `can_edit` w kontekście (viewer=False, admin=True), blokada POST add-ability dla view-only (403), nadal blokowany stranger na armii prywatnej. Pytest 275/275.
- [x] H.5: Weryfikacja live (izolowana kopia bazy, użytkownik nie-admin nie-właściciel armii globalnej): link „Lista zaklęć" widoczny na `/armies/1`, `/armies/1/spells` renderuje 6 pozycji z trudnością/kosztem/nazwą, zero przycisków edycji/usuwania/przeciągania, brak karty dodawania, `POST add-ability` nadal 403. Stan bitewny (`/rosters/10/battle-state`): wszystkie 6 pozycji pokazują `<span class="spell-difficulty">` zaraz po koszcie (np. „1 4+ Formacja marszowa!").

## Faza I — /simplify na Fazie H (4 agenci) 2026-07-10
- Kontekst: Fazy A-G zostały scommitowane poza tą sesją (commit `199858b Pomieszany`, już na origin/Rozwoj) razem z niepowiązanym wątkiem `kolekcja` — stąd `git diff @{upstream}...HEAD` pusty. Zakres tej rundy = wyłącznie niescommitowana Faza H (`armies.py`, `army_edit.html`, `army_spells.html`, `roster_battle_state.html`).
- [x] I.1 (efficiency): `_spell_page_context` budowało `ability_options`/`passive_definitions` bezwarunkowo — dla view-only użytkowników to realne marnotrawstwo (`ability_registry.definition_payload` robi `sync_definitions` + skan całej tabeli `abilities` na każde żądanie), a szablon i tak czyta je tylko w bloku `{% if can_edit %}`. Fix: `can_edit` liczone najpierw, listy budowane tylko gdy `can_edit=True` (inaczej `[]`).
- [x] I.2 (simplification): kolumna „Nazwa" — 3-way branch (`not can_edit` / `weapon` / edytowalny) z dwoma niemal identycznymi `<div class="form-control ... bg-light">` scalony do 2-way (`can_edit and kind!='weapon'` → formularz; inaczej jeden div z warunkiem inline). Efekt uboczny (poprawka spójności): widz oglądający moc typu broń widzi teraz opis broni zamiast pustej nazwy własnej.
- [x] I.3 (simplification): podwójne zagnieżdżenie `{% if can_edit %}{% if remaining_slots... %}...{% else %}...{% endif %}{% endif %}` scalone do jednego łańcucha `{% if not can_edit %}{% elif ... %}...{% else %}...{% endif %}`.
- Reuse + altitude agenci: bez zastrzeżeń (`_army_can_edit` to prawidłowa deduplikacja 2 miejsc, wzorzec `can_edit` w szablonie spójny z `roster_edit.html`/`roster_strategic_cards.html`, `spell.difficulty` poprawnie reużywa istniejące pole z `_army_spell_entries`). Drobna kosmetyczna uwaga (pusty `<td>` akcji dla widza) — pominięta, nieszkodliwa.
- [x] I.4: nowy test `test_spell_page_context_skips_ability_catalog_for_view_only_user` (regresja na I.1). Smoke TestClient: admin widzi pełne UI edycji (formularz, drag handle, edit/usuń, sortable.js), widz widzi treść (nazwa mocy) bez żadnej kontrolki edycji, w tym poprawiony div „Nazwa" z bg-light. Pytest 276/276.

## Faza J — /code-review + /security-review 2026-07-10
- [x] J.1: `/code-review` (xhigh, 10 finder angles) na Fazie H. 8/10 równoległych agentów padło na limit sesji platformy (nie błąd w kodzie) — pokryłem brakujące kąty (A-E, altitude route-coverage) ręcznie: przeanalizowałem wszystkie 6 call sites `_spell_page_context`, zweryfikowałem De Morgan-równoważność `_army_can_edit` ze starą logiką, potwierdziłem 100% pokrycie edit-access na 12/12 trasach mutujących pod `/spells/...`, `spell.difficulty` zawsze obecne w `_army_spell_entries`. Znaleziono 1 finding: pusty `<td>` akcji dla widza zamiast placeholdera „Tylko podgląd" (wzorzec z `unit_row` w `army_edit.html`) — zgłoszony, nie naprawiony (kosmetyczny, niska priorytet).
- [x] J.2: `/security-review` na pełnym working tree (oba wątki wymieszane). Znaleziono i naprawiono **1 finding HIGH** (poza zakresem tego wątku, w pliku `kolekcja`): IDOR w `app/routers/collections.py` — `POST /collections/units/{unit_id}/models/add` bez sprawdzenia własności jednostki (sąsiedni `GET /collections/units/{unit_id}` je miał). Fix: nowy `_require_unit_access` helper (lustro `_require_owner`), użyty w obu miejscach. Regresja: `tests/test_collections_authz.py` (3 testy, potwierdzone przez `git stash` że failują bez fixa). Udokumentowane w `HANDOFF_kolekcja.md` (plik zablokowany przez inny wątek) + cross-thread nota w `HANDOFF.md`.
- Pytest po obu fazach: 280/280.
